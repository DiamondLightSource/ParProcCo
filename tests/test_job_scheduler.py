...
class TestJobScheduler(unittest.TestCase):
    ...
    def test_old_output_timestamps(self) -> None:
        with TemporaryDirectory(prefix="test_dir_", dir=self.base_dir) as working_directory:
            ...
            self.assertFalse(
                any(job_stats.values()), msg=f"All jobs not failed:" f"{js.job_completion_status.values()}\n"
            )
            self.assertEqual(
                len(job_stats),
                4,
    ...
    @parameterized.expand([("is_lowercase", CLUSTER_QUEUE), ("is_uppercase", CLUSTER_QUEUE.upper())])
    ...
    @parameterized.expand(
        [("all_true", True, True, True), ("all_false", False, False, False), ("true_false", True, False, False)]
    )
        ...
        [
            (
                ...
                convert_to_statusinfo(
                    [
                        [None, Path(f"to/somewhere_{i}"), i, {"terminating_signal": "SIGKILL"}, 8, "FAILED"]
                        for i in range(2)
                    ]
                ),
                convert_to_statusinfo(
            ...
            (
                "none_killed",
                convert_to_statusinfo(
                    [[None, Path("to/somewhere"), i, {"terminating_signal": "0"}, 8, "FAILED"] for i in range(2)]
                ),
                [],
            ),
            (
                ...
                convert_to_statusinfo(
                    [
                        [None, Path("to/somewhere_0"), 0, {"terminating_signal": "SIGKILL"}, 8, "FAILED"],
    ...
    @parameterized.expand(
        [
            (
                "all_success",
                False,
                convert_to_statusinfo([[None, Path(), i, {"terminating_signal": "0"}, 7, "DONE"] for i in range(4)]),
            ...
            (
                ...
                False,
                convert_to_statusinfo(
                    [[None, Path(), i, {"terminating_signal": "SIGKILL"}, 8, "FAILED"] for i in range(4)]
                ),
            ...
            (
                ...
                convert_to_statusinfo(
                    [
                        [None, Path(), 0, {"terminating_signal": "SIGKILL"}, 8, "FAILED"],
    ...
    def test_rerun_killed_jobs(
        self, name, allow_all_failed, job_history, job_completion_status, runs, indices, expected_success, raises_error
    ):
        with TemporaryDirectory(prefix="test_dir_", dir=self.base_dir) as working_directory:
            cluster_output_dir = Path(working_directory) / "cluster_output"
            input_path, output_paths, _, slices = setup_data_files(working_directory, cluster_output_dir)
